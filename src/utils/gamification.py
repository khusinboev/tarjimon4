...
class GamificationEngine:
    ...
    def add_xp(cls, user_id: int, amount: int, reason: str = "") -> Dict[str, Any]:
        ...
        try:
            # First, ensure user exists in enhanced table
            sql.execute("""
            ...
            # Update leaderboard
            sql.execute("""
                INSERT INTO leaderboard (user_id, total_xp, last_updated)
                VALUES (%s, %s, NOW())
    ...
    def check_streak(cls, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            sql.execute("""
                UPDATE users_enhanced 
                SET streak_days = %s, longest_streak = %s, last_streak_date = %s
                WHERE user_id = %s
...
class AchievementManager:
    ...
    def check_achievements(user_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            sql.execute("""
                SELECT 
                    ue.translations_count,
                    ue.vocab_books_count,
                    ue.words_count,
                    ue.streak_days,
                    ue.practice_sessions
                FROM (
                    SELECT 
                        ...
                        (SELECT COUNT(*) FROM study_sessions WHERE user_id = %s) as practice_sessions
                ) ue
...
class DailyChallengeManager:
    ...
    def generate_daily_challenge(cls) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            sql.execute("SELECT id FROM daily_challenges WHERE date = CURRENT_DATE")
            if sql.fetchone():
                return None
            ...
            sql.execute("""
                ...
                (date, title, description, challenge_type, target_value, reward_xp)
    ...
    def update_progress(cls, user_id: int, challenge_type: str, amount: int = 1):
        ...
        try:
            sql.execute("""
                SELECT dc.id, dc.target_value, dc.reward_xp
                FROM daily_challenges dc
                WHERE dc.date = CURRENT_DATE AND dc.challenge_type = %s
            """, (challenge_type,))
            ...
            sql.execute("""
                ...
                SET current_value = LEAST(user_daily_challenges.current_value + %s, %s)
                RETURNING current_value
            """, (user_id, challenge_id, amount, amount, target))
...
class LeaderboardManager:
    ...
    def update_rankings():
        ...
        try:
            # Update ranks based on total XP
            sql.execute("""
                WITH ranked AS (
                ...
                SET current_rank = r.new_rank,
                    highest_rank = LEAST(l.highest_rank, r.new_rank),
                    last_updated = NOW()
                FROM ranked r
    ...
    def get_user_rank(user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "xp": result[1],
                "total_users": total,
                "percentile": (1 - result[0] / total) * 100 if total > 0 else 0
            }
        except Exception as e:
