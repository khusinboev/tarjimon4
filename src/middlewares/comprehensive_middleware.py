...
class ComprehensiveUserMiddleware(BaseMiddleware):
    ...
    async def _process_user_activity(self, user, event_type):
        ...
        try:
            sql.execute("SELECT id FROM users WHERE user_id = %s", (user_id,))
            ...
            if not existing:
                ...
                sql.execute("""
                    UPDATE users SET
                        ...
                        last_activity_at = %s, updated_at = %s, is_active = 1
                    WHERE user_id = %s
                """, (user.first_name, user.last_name, user.username,
                      user.language_code, now_str, now_str, user_id))

            # daily activity
            sql.execute("SELECT id FROM user_activity_daily WHERE user_id=%s AND activity_date=%s", (user_id, today))
            if not sql.fetchone():
                sql.execute("INSERT INTO user_activity_daily (user_id, activity_date) VALUES (%s,%s)", (user_id, today))
            ...
            """, (user_id, thirty_ago))
            if not sql.fetchone():
                sql.execute("INSERT INTO user_sessions (user_id, started_at) VALUES (%s,%s)", (user_id, now_str))
