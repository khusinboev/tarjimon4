...
async def daily_info(callback: CallbackQuery):
    ...
    await callback.answer("Kunlik vazifa!", show_alert=True)
...
async def daily_progress(callback: CallbackQuery):
    ...
    await callback.answer("Davom eting!", show_alert=True)
...
async def daily_claim(callback: CallbackQuery):
    ...
    await callback.message.answer(
        "🎉 <b>TABRIKLAYMIZ!</b>\n\n"
        "Siz kunlik vazifani bajardingiz!",
        parse_mode="HTML"
    )
    await callback.answer("Mukofot olingan!", show_alert=True)
