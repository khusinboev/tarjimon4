...
from config import ADMIN_ID, DB_TYPE
...
async def create_parallel_tables():
    ...
    await db_exec(f"""
                  ...
                  (
                      ...
                      trg_lang VARCHAR(10) NOT NULL,
                      icon VARCHAR(20),
                      is_active {boolean_type} DEFAULT {boolean_default},
...
def parallel_main_kb(lang: str) -> InlineKeyboardMarkup:
    ...
    buttons.append([InlineKeyboardButton(text=L["back"], callback_data="parallel:back_to_cabinet")])
...
def create_question_kb(options: list, current_index: int, lang: str) -> InlineKeyboardMarkup:
    ...
    kb_rows.extend([
        ...
        [InlineKeyboardButton(text=L["main_menu"], callback_data="parallel:back_to_cabinet")]
    ])
