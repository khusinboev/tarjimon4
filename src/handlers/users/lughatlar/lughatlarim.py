...
def book_detail_kb(book_id: int, is_public: bool, lang: str) -> InlineKeyboardMarkup:
    ...
    buttons.extend([
        ...
        [InlineKeyboardButton(text=L["back"], callback_data="lughat:back_to_list")]
    ])
...
def new_book_cancel_kb(lang: str) -> InlineKeyboardMarkup:
    L = get_locale(lang)
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=L["cancel"], callback_data="lughat:cancel_new")]
    ])
...
async def cb_lughatlarim(cb: CallbackQuery):
    ...
    if not books and page == 0:
        kb = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text=L["back"], callback_data="lughat:back_to_cabinet")]
        ])
...
async def add_book(msg: Message, state: FSMContext):
    ...
    if GAMIFICATION_ENABLED and GamificationEngine and XPRewards:
        try:
            ...
            logging.error(f"Gamification error in create book: {e}")
    ...
    if added_count > 0:
        ...
        if GAMIFICATION_ENABLED and GamificationEngine and XPRewards:
            try:
                ...
                logging.error(f"Gamification error in add words: {e}")
