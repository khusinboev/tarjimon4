...
LOCALES = {
    "uz": {
        ...
        "ask_add_after_create": "Lug'at yaratildi. So‘zlarni hozir qo‘shmoqchimisiz?",
        "yes_add": "➕ Ha, qo'shaman",
        "no_later": "🔙 Keyinroq",
        "export_excel": "📤 Excelga eksport",
        "manage_book": "⚙️ Lug'atni boshqarish",
        "delete_pair_prompt": "O'chirmoqchi bo'lgan juftlikni yuboring (word-translation):",
        "pair_deleted": "✅ Juftlik o'chirildi.",
        "pair_not_found": "❌ Bunday juftlik topilmadi.",
        "no_entries": "Bu lug'atda so'zlar yo'q.",
    ...
    "en": {
        ...
        "back_to_book": "🔙 Back",
        "main_menu": "🏠 Main menu",
        "ask_add_after_create": "Book created. Add words now?",
        "yes_add": "➕ Yes, add",
        "no_later": "🔙 Later",
        "export_excel": "📤 Export to Excel",
        "manage_book": "⚙️ Manage book",
        "delete_pair_prompt": "Send the pair to delete (word-translation):",
...
def book_kb(book_id: int, lang: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=L["manage_book"], callback_data=f"book:manage:{book_id}")],
...
def books_list_kb(books: List[Dict[str, Any]], lang: str, page: int = 1) -> InlineKeyboardMarkup:
    L = LOCALES[lang]
    start = (page - 1) * PAGE_SIZE
    end = start + PAGE_SIZE
    chunk = books[start:end]
    buttons = [[InlineKeyboardButton(text=b["name"], callback_data=f"book:open:{b['id']}")] for b in chunk]
...
@router.callback_query(lambda c: c.data and c.data.startswith("book:add_after_create:"))
async def cb_add_after_create(cb: CallbackQuery):
    ...
    # set state and store book_id
    state = cb._bot['fsm_context'] if hasattr(cb, '_bot') else None
    # we cannot access FSMContext here easily, so ask user to send command instead
    # Simpler: set a hidden state by asking them to press Add words from book menu


# -------------------- Book open / manage --------------------

@router.callback_query(lambda c: c.data and c.data.startswith("book:open:") or c.data and c.data.startswith("book:manage:"))
async def cb_book_open(cb: CallbackQuery, state: FSMContext):
    # unified handler for open & manage
    parts = cb.data.split(":")
    action = parts[1]
    book_id = int(parts[2])
    lang = await get_user_lang(cb.from_user.id)
...
@router.callback_query(lambda c: c.data and c.data.startswith("book:manage:"))
async def cb_book_manage(cb: CallbackQuery):
    book_id = int(cb.data.split(":")[2])
    lang = await get_user_lang(cb.from_user.id)
    L = LOCALES[lang]
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=L["export_excel"], callback_data=f"book:export:{book_id}")],
        [InlineKeyboardButton(text=L["add_words"], callback_data=f"book:add:{book_id}")],
        [InlineKeyboardButton(text=L["delete"], callback_data=f"book:delete_confirm:{book_id}")],
        [InlineKeyboardButton(text=L["back"], callback_data=f"book:open:{book_id}")]
    ])
    try:
        await cb.message.edit_text(L["manage_book"], reply_markup=kb)
    except Exception:
        await cb.message.answer(L["manage_book"], reply_markup=kb)
...
@router.callback_query(lambda c: c.data and c.data.startswith("mybooks:page:"))
async def cb_mybooks_page(cb: CallbackQuery):
...
@router.callback_query(lambda c: c.data and c.data.startswith("book:delete_pair:"))
async def cb_book_delete_pair(cb: CallbackQuery, state: FSMContext):
    book_id = int(cb.data.split(":")[2])
    lang = await get_user_lang(cb.from_user.id)
    L = LOCALES[lang]
    await cb.message.edit_text(L["delete_pair_prompt"], reply_markup=add_words_back_kb(book_id, lang))
    await state.update_data(book_id=book_id)
    await state.set_state(VocabStates.waiting_delete_pair)


@router.message(VocabStates.waiting_delete_pair)
async def delete_pair(msg: Message, state: FSMContext):
    user_id = msg.from_user.id
    lang = await get_user_lang(user_id)
    L = LOCALES[lang]
    data = await state.get_data()
